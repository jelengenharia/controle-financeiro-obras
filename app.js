...
const App = (() => {
  ...
  function onNotaSelected(file){
    ...
    reader.onload=e=>{ $("notaImg").src=e.target.result; $("notaPreview").classList.remove("hidden"); };
  ...
  function clearNota(){
    notaUploaded={fileId:"",url:""};
    const p=$("notaPreview"); if(p) p.classList.add("hidden");
  ...
  async function adminWorkUpsert(){
    try{
      const work={obra_id:$("aw_id").value.trim(),obra_nome:$("aw_nome").value.trim(),ativa:$("aw_  async function adminWorkUpsert(){
    try{
